...
ggplot(data = mpg) + 
      geom_point(color = 'blue',  mapping = aes(x = displ, y = hwy)) + facet_grid(class ~ drv)
...
ggplot(data = mpg, aes(x = cty, y = hwy, color = class)) + geom_point(size = 6) + 
...
subcompact_mpg_15mph <- mpg[mpg$class == "subcompact" & mpg$cty == 15, c("manufacturer", "model")] 
subcompact_mpg_15mph
...
# Write your code here
ggplot(data = mpg) + 
      geom_point(mapping = aes(x = displ, y = hwy, shape = class)) + scale_shape_manual(values = 1:7)
...
colnames(sizeMass)[1] = "size"
colnames(sizeMass)[2] = "mass"
sizeMass
```
...
ggplot(sizeMass, aes(size, mass)) +
      geom_point() +
      geom_smooth(method = "lm", color = "red", se = FALSE) + scale_x_log10() + scale_y_log10()
